...
export default function TrackingPage({ params }: { params: Promise<{ tracking: string }> }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/ratings/${encodeURIComponent(tracking)}`, {
      method: "POST",
      headers: {
      ...
      .then(async (response) => {
        if (!response.ok) {
          throw new Error(`Rating save failed: ${response.status}`)
        }
        if (cancelled) return
        ratingSyncRef.current = syncKey
        setRatingState("saved")
  ...
  if (showRatingPage && order.status === "delivered" && !submitted) {
    async function handleSubmitRating() {
      ...
      setSubmitting(true)
      try {
        const ratingToSubmit = serviceRating > 0 ? serviceRating : order.customerRating ?? 5
        const res = await fetch(`/api/ratings/${encodeURIComponent(tracking)}`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
          body: JSON.stringify({
            customerRating: ratingToSubmit,
            ...(driverRating > 0 ? { driverRating } : {}),
            ...(feedback.trim() ? { customerFeedback: feedback.trim() } : {}),
          }),
        })
