...
import { adminFetchOrderByTracking, adminUpdateOrder, adminRecalculateDriverRating } from "@/lib/server/firestore-admin"
...
export async function POST(
  ...
  try {
    const order = await adminFetchOrderByTracking(tracking)
    if (!order) {
    ...
    if (order.assignedDriver) {
      adminRecalculateDriverRating(order.assignedDriver).catch(() => {})
    }
