...
function getMissingNotificationEnvKeys() {
  const missing: string[] = NOTIFICATION_ENV_KEYS.filter((key) => !process.env[key] || process.env[key]?.trim() === "")

  if (!process.env.TWILIO_WHATSAPP_FROM && !process.env.TWILIO_WHATSAPP_MESSAGING_SERVICE_SID) {
    missing.push("TWILIO_WHATSAPP_FROM or TWILIO_WHATSAPP_MESSAGING_SERVICE_SID")
  }
...
/**
 * Normalize a phone number to E.164 (e.g. "+2348012345678") for Twilio.
 * Handles common Nigerian variants:
 *   "0801 234 5678"     → "+2348012345678"
 *   "8012345678"        → "+2348012345678"
 *   "234 801 234 5678"  → "+2348012345678"
 *   "+234 801 234 5678" → "+2348012345678"
 * Returns null when input has no usable digits.
 */
function toE164(input: string): string | null {
  ...
  // Other lengths: assume already-international without leading +.
  return `+${digits}`
}

/**
 * Build the Twilio Content Template variable map for a given event.
 * Configure your approved template using these positional placeholders:
 *   {{1}} customer name
 *   {{2}} order number
 *   {{3}} status text  ("is on the way", "has been delivered", etc.)
 *   {{4}} tracking URL
 */
function buildWhatsappContentVariables(
  event: OrderEvent,
  ...
  return {
    "1": payload.customerName?.trim() || "Customer",
    "2": payload.orderNumber,
    "3": statusText[event] ?? "has an update",
    "4": payload.trackingUrl ?? "",
  }
}
...
function buildMessage(event: OrderEvent, payload: NotificationPayload) {
  ...
  if (event === "out_for_delivery") {
    // Use the customer's requested plain message for outbound notifications
    const tracking = payload.trackingUrl ? `\n\n${payload.trackingUrl}` : ""
    // WhatsApp/plain-text friendly message
    const whatsappMessage = `Hello ${payload.customerName},\nThank you for shopping with Sterlin Glams. We’re happy to let you know that your order is now out for delivery.${tracking}\n\nThank you for choosing Sterlin Glams.\n\nWarm regards,\nSterlin Glams Logistics`

    // HTML/email friendly message — keep existing summary and branding but replace body text
    // The email template below will still render the Track Your Order button if trackingUrl exists.
...
function getWhatsappContentSid(event: OrderEvent) {
  const eventKey = event.toUpperCase()
  return (
    process.env[`TWILIO_WHATSAPP_CONTENT_SID_${eventKey}`] ||
...
async function sendTwilioMessage(
  ...
  const whatsappMessagingServiceSid = process.env.TWILIO_WHATSAPP_MESSAGING_SERVICE_SID

  if (!sid || !token) return { sent: false, reason: "missing_twilio_credentials" }

  // Normalize the destination to E.164 so Twilio doesn't reject the request.
  const toE164Number = toE164(to)
  if (!toE164Number) {
    return { sent: false, reason: `invalid_${channel}_to_number` }
  }

  const from = channel === "sms" ? smsFrom : whatsappFrom
  const usesWhatsappMessagingService = channel === "whatsapp" && whatsappMessagingServiceSid

  if (!from && !usesWhatsappMessagingService) {
    return {
      sent: false,
  ...
  if (usesWhatsappMessagingService) {
    params.set("MessagingServiceSid", whatsappMessagingServiceSid)
  } else if (from) {
    params.set("From", channel === "whatsapp" ? asWhatsappNumber(from) : from)
  }
  params.set("To", channel === "whatsapp" ? asWhatsappNumber(toE164Number) : toE164Number)

  if (channel === "whatsapp" && options?.contentSid) {
    params.set("ContentSid", options.contentSid)
    if (options.contentVariables) {
      params.set("ContentVariables", options.contentVariables)
    }
  } else {
...
export async function sendOrderEventNotifications(
  ...
  const whatsappContentSid = getWhatsappContentSid(event)
  const subject =
    event === "order_accepted"
  ...
  const [sms, whatsapp, email] = await Promise.all([
    ...
    doWhatsapp
      ? sendTwilioMessage("whatsapp", payload.customerPhone, body, {
          ...
          contentVariables: whatsappContentSid
            ? JSON.stringify(buildWhatsappContentVariables(event, payload))
            : undefined,
        })
      : Promise.resolve({ sent: false, reason: "disabled_by_settings" }),
